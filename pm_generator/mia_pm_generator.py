...
def mia_pm_generator(model_name, model_path, dir_path, data_path, subjects, save_image=False, test_mode=False):
    ...
    for idx, (image, target) in enumerate(dataset):
        ...
        if save_image:
            ...
            if not os.path.isdir(os.path.join('./result_not_mia', subject)):
                os.mkdir(os.path.join('./result_not_mia', subject))
            cv2.imwrite(os.path.join('./result_not_mia', subject, img_num), pm)
